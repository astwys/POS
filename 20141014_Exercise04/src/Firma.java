...
/**
 * implemented new methods sortieren...()
 * @author michael
 *
 */
public class Firma //extends Mitarbeiter
{
	...
	public void sortieren()
	{
		...
		{
			for (int j=0; j<mitarbeiter.size()-1; j++)
			{
				if (mitarbeiter.get(j).berechneGehalt()<mitarbeiter.get(j+1).berechneGehalt())
				{
					temp=mitarbeiter.get(j);
					mitarbeiter.set(j, mitarbeiter.get(j+1));
					mitarbeiter.set(j+1, temp);
				}
			}
		}
	}
	...
	public void sortierenComparable()
	{
		Mitarbeiter temp;
		...
		for (int i=0; i<mitarbeiter.size()-1; i++)
		{
			for (int j=0; j<mitarbeiter.size()-1; j++)
			{
				if (mitarbeiter.get(j).compareTo(mitarbeiter.get(j+1))<0)
				{
					temp=mitarbeiter.get(j);
					mitarbeiter.set(j, mitarbeiter.get(j+1));
					mitarbeiter.set(j+1, temp);
	...
	public void sortierenComporable (boolean aufsteigend)
	{
		Mitarbeiter temp;
